...
private:
  ...
  guint               channel_buffer_size_;       // = n_frames + 1; the extra frame allows us to
...
public:
        ...
        T     **frames)
  {
    ...
    guint read1 = min (can_read, channel_buffer_size_ - rpos);
    guint read2 = can_read - read1;

    for (guint ch = 0; ch < n_channels_; ch++)
      {
         ...
         const T **frames)
  {
    ...
    guint write1 = min (can_write, channel_buffer_size_ - wpos);
                     ...
                     guint        *wlatency)
{
  ...
  guint total_latency = jack->buffer_frames + jack_rlatency + jack_wlatency;
                  ...
                  gfloat       *values)
{
  ...
  guint frames_read = jack->input_ringbuffer.read (handle->block_length, deinterleaved_frames);
                   ...
                   const gfloat *values)
{
  ...
  guint frames_written = jack->output_ringbuffer.write (handle->block_length, deinterleaved_frames);
